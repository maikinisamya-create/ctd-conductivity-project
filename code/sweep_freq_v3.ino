...
//
// Outil de diagnostic / calibration : balaie une plage de frequences fixe
// (SWEEP_FREQ_START a SWEEP_FREQ_STOP, pas SWEEP_INCR_HZ) sur la cellule de
// conductivite CN0349/AD5934, et affiche sur le port serie un tableau complet
// (frequence, conductivite, salinite PSS-78, conductivite a 25 C) pour
// chaque point mesure. Contrairement au code de mesure adaptatif, aucune
// frequence n'est choisie automatiquement : c'est a l'utilisateur d'
// interpreter le tableau pour reperer la frequence la plus stable/fiable
// pour l'echantillon d'eau teste (utile avant un deploiement, ou pour
// construire le modele frequence/conductivite utilise par le code adaptatif).
//
// Sketch autonome : il contient sa propre copie du pilotage bas niveau
// AD5934/ADG715, du calcul de salinite PSS-78 (sans correction de pression)
// et de la gestion RTC/SD/temperature -- il n'utilise pas les headers
// AD5934_CN0349.h / PSS78.h du code adaptatif. Gamme fixee HIGH (eau
// conductrice type eau de mer).
//
// Capteurs/peripheriques utilises : AD5934+ADG715 (CN0349), TSYS01
// (temperature), DS3231M (horloge), carte SD (relecture uniquement, ce
// sketch n'ecrit pas de donnees). Voir le README du depot pour le cablage
// complet et le detail des parametres du sweep.
//
// COMMANDES SERIE (115200 bauds) :
//   r  --> lire fichier de donnees SD (ecrit par le code adaptatif, pas par ce sweep)
