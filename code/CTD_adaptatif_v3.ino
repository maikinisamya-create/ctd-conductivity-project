...
//
// Mesure autonome de conductivite / temperature / profondeur (CTD) en mer,
// destinee a un deploiement de terrain sans operateur (bouee, capteur
// immerge). A chaque cycle, le code :
//   1. mesure la temperature (TSYS01) et la pression/profondeur (MS5837 Bar30) ;
//   2. cherche automatiquement la frequence de mesure optimale sur la cellule
//      de conductivite CN0349/AD5934, par iterations successives jusqu'a
//      convergence (pas de frequence fixe : elle s'adapte a la conductivite
//      reelle de l'eau) ;
//   3. calcule la salinite pratique (PSS-78, Fofonoff & Millard, UNESCO 1983,
//      avec correction de temperature et de pression) ;
//   4. enregistre le resultat sur carte SD (CSV) et journalise les eventuelles
//      erreurs dans un fichier log separe ;
//   5. se met en veille quelques secondes avant de recommencer.
//
// Le pilotage bas niveau de la puce AD5934/ADG715 et le calcul PSS-78 sont
// factorises dans deux fichiers a copier dans le meme dossier que ce sketch :
//   AD5934_CN0349.h -- calibration/mesure d'impedance, I2C bas niveau
//   PSS78.h         -- calculerSP() / calculerC25()
//
// Capteurs/peripheriques utilises : AD5934+ADG715 (CN0349), TSYS01
// (temperature), MS5837 Bar30 (pression/profondeur), DS3231M (horloge),
// carte SD (donnees + log). Voir le README du depot pour le cablage complet,
// les bibliotheques a installer et le detail de chaque fonction.
//
// COMMANDES SERIE (115200 bauds) :
//   r  --> lire fichier de donnees SD du jour
//   l  --> lire fichier de log erreurs SD du jour
//   T  --> regler RTC : T20260616102500 (T + AAAAMMJJHHMMSS)
// ═══════════════════════════════════════════════════════════════════════════
...
ResultatCycle mesureAdaptative() {
    ...
    if(!m1.valide||m1.sigma_mScm<SIGMA_SEUIL_GAMME){
        ...
        if(!m1.valide){
            // Pas une erreur : cas normal si la sonde est allumee avant d'etre
            // immergee (a l'air le temps de la poser a l'eau). On affiche juste
            // l'info sur le port serie, sans la journaliser dans le fichier log.
            Serial.println("[INFO] Sonde hors eau -- sigma=0 stocke");
